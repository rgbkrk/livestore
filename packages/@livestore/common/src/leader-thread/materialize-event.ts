...
export const makeMaterializeEvent = ({
  ...
  Effect.gen(function* () {
    const eventDefSchemaHashMap = new Map(
      ...
      [...schema.eventsDefsMap.entries()].map(
        ([k, v]) => [k, Schema.hash(v.schema)] as const,
      ),
    );
    ...
    return (eventEncoded, options) =>
      Effect.gen(function* () {
        ...
        const materializerHash = isDevEnv()
          ? Option.some(hashMaterializerResults(execArgsArr))
          : Option.none();

        if (
          materializerHash._tag === "Some" &&
          eventEncoded.meta.materializerHashSession._tag === "Some" &&
          eventEncoded.meta.materializerHashSession.value !==
            materializerHash.value
        ) {
        ...
        if (skipEventlog === false) {
          ...
          const eventDefSchemaHash =
            eventDefSchemaHashMap.get(eventName) ??
            shouldNeverHappen(`Unknown event definition: ${eventName}`);
...
export const rollback = ({
  ...
  Effect.gen(function* () {
    ...
    const eventNumPairChunks = ReadonlyArray.chunksOf(100)(
      eventNumsToRollback.map(
        (seqNum) => `(${seqNum.global}, ${seqNum.client})`,
      ),
    );
